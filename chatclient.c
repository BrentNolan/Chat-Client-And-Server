...
struct addrinfo * create_address_info(char * input_addr, char * port){
	...
	// if the status is an error (not 0), print it
	// else, return the address information
...
int create_socket(struct addrinfo * res){
	int sockfd;
	// if the socket file descriptor is -1, exit, else return it
...
void connect_socket(int sockfd, struct addrinfo * res){
	...
	// else connect the socket to the address information
...
/*******************************************************************************
void hand_shake(int, char*, char*)
 
intiates an exhange of information between the client and the server. 
Exchanges names between server and client
Args: the user's name and an empty array to store the server's name
 ******************************************************************************/
void hand_shake(int sockfd, char * user, char * server){
	// send our user to the peer
	int sendingcode = send(sockfd, user, strlen(user), 0);
	// get the user of our peer and place into server
	int receivingcode = recv(sockfd, server, 10, 0);
}
...
void chat(int sockfd, char * username, char * servername){
	// create and clear buffers for input and output
	char input[503];
	char output[501];
...
int main(int argc, char *argv[]){
	...
	// create and connect socket from address information from the arguments 
	struct addrinfo * res = create_address_info(argv[1], argv[2]);
	int sockfd = create_socket(res);
	connect_socket(sockfd, res);
	
	// create and exhange username and peername and start chat
	char server[10];
	hand_shake(sockfd, user, server);
	chat(sockfd, user, server);
